...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected SPRY and zSPRY supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zSPRYsupply']:
                assert_equal(node['zSPRYsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check SPRY and zSPRY supply at the beginning
        # ------------------------------------------
        # zSPRY supply: 2 coins for each denomination
        ...
        # SPRY supply: block rewards minus burned fees for minting
        ...
        # Verify that SPRY and zSPRY supplies were properly updated after the spends and reorgs
        self.log.info("Check SPRY and zSPRY supply...")
