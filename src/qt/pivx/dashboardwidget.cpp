    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of SPRY earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 SPRY");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("SPRY")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair SPRY, zSPRY
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(108, 120, 255));
	set1->setColor(QColor(0, 157, 255));
