...
const char * const PIVX_CONF_FILENAME = "sprycoin.conf";
const char * const PIVX_PID_FILENAME = "sprycoin.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Sprycoin only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\sprycoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\sprycoin
// Mac: ~/Library/Application Support/sprycoin
// Unix: ~/.sprycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sprycoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\sprycoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\sprycoinParams
    // Mac: ~/Library/Application Support/sprycoinParams
    // Unix: ~/.sprycoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SprycoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SprycoinParams";
#else
    // Unix
    return pathRet / ".sprycoin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty sprycoin.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Sprycoin config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override sprycoin.conf
